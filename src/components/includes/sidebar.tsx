...
const Sidebar = (props: SidebarProps) => {
  ...
  query {
    allMdx(sort: {fields: frontmatter___position, order: ASC}, filter: {fields: {url: {ne: "/404/"}}}) {
      nodes { frontmatter { title description position } fields { url } }
    }
  }`);
  ...
  const renderItem = (node: Node, depth: number) => {
    ...
    return <React.Fragment key={node.url}>
      <Item title={node.title} url={node.url} child={depth > 0} current={props.current} />
      { children.length > 0 && (props.current.indexOf(node.url) === 0 || depth < 1) && <nav className="nav nav-pills flex-column ml-3">
        {_.sortBy(children, "position").map(node => renderItem(node, depth + 1))}
      </nav> }
    </React.Fragment>;
  }

  return <nav className="nav flex-column nav-pills">
    {renderItem(index, 0)}
    {_.sortBy(Object.values(navigation), "position").map((node: Node) => <React.Fragment key={node.url}>
...
const Item = (props: ItemProps) => {
  ...
  return <a className={className} href={withPrefix(props.url)}>{props.title}</a>;
}
